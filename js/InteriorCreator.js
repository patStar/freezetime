function InteriorCreator() {

    this.createCabinRoom = function () {
        var interior = [];
        ...
        for (var n = 0; n < 3; n++) {
            if (w(2)) {
                ...
                if (furniture == Indoor.stoneHearth) {
                    interior.push(new InteriorPiece(furniture.instantiate(), n * 128 + 32, 32, InteriorPiece.TYPE.big));
                    arrayRemove(Indoor.stoneHearth, bigFurnitures);
                } else if (furniture == Indoor.shelf) {
                    interior.push(new InteriorPiece(furniture.instantiate(), n * 128 + 32, 72, InteriorPiece.TYPE.big));
                }
            } else {
                // big furniture
                ...
                if (furnitureTop == Indoor.map) {
                    interior.push(new InteriorPiece(furnitureTop.instantiate(), n * 128 + 32 - 4, 72, InteriorPiece.TYPE.smallTop));
                ...
                if (furnitureBottom == Indoor.bigTable) {
                    interior.push(new InteriorPiece(furnitureBottom.instantiate(), n * 128 + 16, 32 * 5 + 16, InteriorPiece.TYPE.smallBottom));
                    arrayRemove(furnitureBottom, smallFurnituresBottom);
                } else if (furnitureBottom == Indoor.smallShelf) {
                    interior.push(new InteriorPiece(furnitureBottom.instantiate(), n * 128 + 32, 32 * 5 - 12, InteriorPiece.TYPE.smallBottom));
                }
            }
        }
